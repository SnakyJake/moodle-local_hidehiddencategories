...
function xmldb_local_hidehiddencategories_uninstall(){
    global $DB;

    require_once("_services.php");

    foreach($functions as $name=>$params){
        $record = $DB->get_record('hidehiddencategories_backup', array('name'=>$name), '*', MUST_EXIST);
        $DB->delete_records('external_functions', array('name'=>$name));
        $DB->insert_record('external_functions',$record);
    }
